    ...
    for col in df.select_dtypes(include=['number']).columns:
        if not df[col].isnull().all():
            numeric_stats[col] = {
                ...
                'std': float(df[col].std()) if len(df[col].dropna()) > 1 else 0
            }
    
    # Check for trajectory data
    trajectory_metrics = {}
    if all(col in df.columns for col in ['position_n', 'position_e', 'position_d']):
        # Calculate distance metrics if position columns exist
        try:
            ...
            if len(df_clean) > 1:
                ...
                for i in range(1, len(df_clean)):
                    ...
                    p2 = (df_clean.iloc[i]['position_n'], df_clean.iloc[i]['position_e'], df_clean.iloc[i]['position_d'])
                ...
                # Altitude change
                min_alt = -df_clean['position_d'].max()  # Negative of max 'down' is min altitude
                max_alt = -df_clean['position_d'].min()  # Negative of min 'down' is max altitude
...
def apply_cleaning_operations(file_info, config):
    ...
    if config.get('file_size_filtering', {}).get('enabled', False):
        ...
        if file_info['file_size_mb'] < min_size_mb:
            ...
            result['cleaning_results']['file_size_filtering'] = {
                'action': 'skipped',
                'reason': f'File size {file_info["file_size_mb"]:.2f} MB is below threshold {min_size_mb} MB'
            }
        ...
        if 'velocity' in df.columns:
            ...
            if all(col in df.columns for col in ['position_n', 'position_e', 'position_d']):
                ...
                for i in range(1, len(df)):
                    p1 = np.array([df.iloc[i-1]['position_n'], df.iloc[i-1]['position_e'], df.iloc[i-1]['position_d']])
                    p2 = np.array([df.iloc[i]['position_n'], df.iloc[i]['position_e'], df.iloc[i]['position_d']])
    ...
    if config.get('anomaly_detection', {}).get('enabled', False) and not df.empty:
        ...
        if 'trajectory_metrics' in file_info:
            ...
            if 'altitude_change' in metrics and 'altitude_change' in metrics_to_check:
                alt_change = metrics['altitude_change']
    ...
    if not df.empty:
        ...
        cleaned_preview = df.head(5).to_dict('records')
    else:
        cleaned_content = ""
    ...
    result['final_stats'] = {
        ...
        'column_count': columns_after,
        'file_size_bytes': cleaned_size_bytes,
        'file_size_mb': cleaned_size_mb,
        'rows_removed': file_info['row_count'] - rows_after,
        'percentage_reduction': ((file_info['row_count'] - rows_after) / file_info['row_count']) * 100 if file_info['row_count'] > 0 else 0,
        'size_reduction': ((file_info['file_size_bytes'] - cleaned_size_bytes) / file_info['file_size_bytes']) * 100 if file_info['file_size_bytes'] > 0 else 0
    }
    
    result['cleaned_preview'] = cleaned_preview
    
    # In an actual implementation, we would save the cleaned file here
    # and update result with the path to the cleaned file
